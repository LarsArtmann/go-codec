...
| Envelope                   | A self-describing JSON wrapper (magic `"cqrs"` + `Encoding` + inner data) that lets a blind store recover the codec used to write a value.          | `envelope.go` — `WrapEncode` / `UnwrapDecode`   |
