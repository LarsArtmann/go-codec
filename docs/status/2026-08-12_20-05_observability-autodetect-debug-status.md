...
_Nothing significant._ The core deliverables are complete and verified. The only "partial" item is the stale diagnostics tooling artifact (see d), which is not user-facing code but degrades trust in the LSP feedback loop.
