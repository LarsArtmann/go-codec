...
> No version has been tagged yet (`git tag` is empty). Everything shipped so far
> lives under `[Unreleased]` until the first tagged release.
...
- Four codec implementations:
  ...
  - `CBORCompactCodec` — Core Deterministic CBOR (RFC 8949, bytewise-lexical
    ...
    `cbor_compact.go`
  - `JSONCodec` — `encoding/json/v2` with deterministic marshal and
    case-insensitive decode — `json.go`
  - `RawCodec` — `[]byte` passthrough (copies on decode) — `raw.go`
- Codec resolution and format detection: `ForEncoding` (encoding stamp → codec)
  and `AutoDetect` (best-effort encoding inference from raw bytes) — `codec.go`,
  `autodetect.go`
...
- Comprehensive test suite: behavioral unit tests, fuzz targets, property tests
  (`pgregory.net/rapid`), golden snapshots (`go-snaps`), benchmarks, and godoc
  `Example*` functions
- Project scaffolding: `README.md`, `CONTRIBUTING.md`, `LICENSE`, `go.mod`,
  editor config, Git attributes, and generated-artifact exclusions

### Changed

- Aligned `.gitignore` ignore rules to the module name (artifact path `/go-codec`)
  and adjusted the snapshot-cleanup call in `TestMain` — `f327816`
