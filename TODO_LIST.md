...
| 8 | ADR: error taxonomy (families per layer), WrapOnce rule, generic_return decline rationale | 🔴 `TODO` | Med | 1h | Input: 2026-09-11 diagnosis — the 11→7 `generic_return` drop was an analyzer false-negative (`createsErrorInternally` exact-matches `Wrap`/`Wrapf`, missing `WrapOncef`/`WrapCorruptionf`/`WrapInfrastructuref`); the decline rationale itself stands. **Direction locked 2026-09-11: typed public errors are a future v2 goal — the 7 declines are TRANSITIONAL, not permanent policy** (record as such in the ADR). Source: report §f-20/§b-1. |
