...
1. Add depth cap (`maxDepth=100`) to `normalizeForJSON` — return `(any, error)`
2. Wire the 3 callers to propagate the error
...
3. Add size guard to `AutoDetect` before trial-decode
4. Fix README `"Go 1.23+"` → `"Go 1.26.5+"`
5. Create GitHub Release for `v0.1.0`
...
6. Add `gosec` + `gitleaks` to CI
7. Add v2-mode lint job to CI
8. Revert `makezero` to `always: true`, add targeted `//nolint`
9. Remove dead `testJSONMarshal` helper
10. Re-enable `tagliatelle` in tests
11. Extract test string constants
...
| --- | ---------------------------------------------------------------------- | ------ | ------- | -------- | --------------- |
| M1  | Add depth cap to `normalizeForJSON`; wire 3 callers to propagate error | Exist  | 30min   | —        | Security        |
| M2  | Add size guard to `AutoDetect` (return `EncodingRaw` on oversized)     | Exist  | 15min   | —        | Security        |
| M3  | Fix README `"Go 1.23+"` → `"Go 1.26.5+"`; create GitHub Release v0.1.0 | High   | 15min   | —        | Release/Docs    |
| M4  | Add `gosec` + `gitleaks` steps to CI                                   | High   | 30min   | —        | CI              |
| M5  | Add v2-mode lint job to CI                                             | High   | 15min   | —        | CI              |
| M6  | Revert `makezero` to `always: true`, add `//nolint` in `raw.go:46`     | Med    | 10min   | —        | Lint debt       |
| M7  | Remove dead `testJSONMarshal`; re-enable `tagliatelle`; extract test constants | Med | 30min | —      | Lint debt       |
| M8  | Add `TestNormalizeForJSON` table-driven test (depends on M1 depth cap) | Med    | 30min   | M1       | Tests           |
| M9  | Add `FuzzNormalizeForJSON` fuzz target (depends on M1)                 | Med    | 15min   | M1       | Tests           |
| M10 | Re-enable `testpackage`; migrate 10 white-box files to `codec_test` + `export_test.go` | Med | 60min | —    | Tests           |
| M11 | Re-enable `paralleltest`; add `t.Parallel()` to 19 funcs (after M10)   | Med    | 30min   | M10      | Tests           |
| M12 | Run all fuzz targets 60s each (v1 + v2); generate coverage (both modes); add % to docs | Med | 45min | —  | Verify          |
| M13 | Verify `flake.nix`: `nix build`, `nix run .#test`, `nix run .#lint`, `nix flake check` | Med | 30min | —  | Verify          |
| M14 | `Size` → `SizeResult{JSON, CBOR int}` struct; update callers + `doc.go` | Low    | 30min   | —        | API             |
| M15 | Document `TranscodeToJSON` one-way contract explicitly (add to doc.go) | Low    | 15min   | —        | API             |
| M16 | Add `sync.Pool[*bytes.Buffer]` helper for `BufferEncoder`              | Low    | 30min   | —        | Perf            |
| M17 | Benchmark v1 vs v2 JSON + `normalizeForJSON` allocation; add numbers to README | Low | 30min | —     | Perf            |
| M18 | Add `CODEOWNERS`, `SECURITY.md`, issue/PR templates                    | Low    | 30min   | —        | Repo            |
| M19 | Decide TIMEZONE_HANDLING (accept inline or create doc); flesh out CONTRIBUTING | Low | 30min | —    | Docs            |
| M20 | Verify `go-cqrs-lite` consumes `go-codec@v0.1.0`                       | Med    | 30min   | —        | Ecosystem       |
| M21 | Update living docs (CHANGELOG, FEATURES, TODO_LIST, AGENTS) after work | Med    | 30min   | M1-M20   | Docs            |
...
| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F1  | Add `maxDepth=100` constant to `json_compat_v1.go`                  | 2min | —      |
| F2  | Change `normalizeForJSON` signature to `(any, error)` + depth param | 8min | F1     |
| F3  | Update `jsonMarshal` to handle `(any, error)` from normalizer       | 3min | F2     |
| F4  | Update `jsonMarshalDet` to handle `(any, error)` from normalizer    | 3min | F2     |
| F5  | Update `jsonMarshalBuf` to handle `(any, error)` from normalizer    | 3min | F2     |
| F6  | Verify: `go build ./...` (v1) passes                                | 2min | F3-F5  |
| F7  | Verify: `GOEXPERIMENT=jsonv2 go build ./...` (v2) passes            | 2min | F6     |
| F8  | Verify: `go test ./... -race` (v1) passes                           | 3min | F7     |
| F9  | Verify: `GOEXPERIMENT=jsonv2 go test ./... -race` (v2) passes       | 3min | F8     |
| F10 | Add size guard (`maxAutoDetectSize`) to `AutoDetect`               | 5min | —      |
| F11 | Verify: `AutoDetect` returns `EncodingRaw` on oversized input       | 3min | F10    |

### Tier 2: README fix + GitHub Release

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F12 | Fix README `"Go 1.23+"` → `"Go 1.26.5+"`                           | 1min | —      |
| F13 | Verify README diff is correct (read surrounding context)           | 2min | F12    |
| F14 | Create GitHub Release for `v0.1.0` (`gh release create`)           | 5min | —      |

### Tier 3: CI Hardening

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F15 | Add `gosec` step to `.github/workflows/ci.yml`                     | 5min | —      |
| F16 | Add `gitleaks` step to `.github/workflows/ci.yml`                  | 5min | —      |
| F17 | Add v2-mode lint job to `.github/workflows/ci.yml`                 | 5min | —      |
| F18 | Verify CI YAML is valid (`gh workflow` lint or yamllint)            | 3min | F15-F17|

### Tier 4: Lint Debt Payback

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F19 | Add `//nolint:makezero` to `raw.go:46` (`cp := make(...)`)         | 2min | —      |
| F20 | Revert `.golangci.yml` `makezero.always` to `true`                 | 2min | F19    |
| F21 | Remove dead `testJSONMarshal` from `json_helpers_v1_test.go`        | 1min | —      |
| F22 | Remove dead `testJSONMarshal` from `json_helpers_v2_test.go`        | 1min | —      |
| F23 | Extract test string constants (`testName`, `testEmail`)             | 8min | —      |
| F24 | Re-enable `goconst` in tests (remove from exclusion list)           | 2min | F23    |
| F25 | Fix `created_at` → `created-at` in test structs OR add nolint       | 3min | —      |
| F26 | Re-enable `tagliatelle` in tests (remove from exclusion list)       | 2min | F25    |
| F27 | Verify: `golangci-lint run ./...` is clean (v1)                    | 5min | F20-F26|
| F28 | Verify: `golangci-lint run --build-tags goexperiment.jsonv2` clean | 5min | F27    |

### Tier 5: Test Package Migration (testpackage)

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F29 | Create `export_test.go` exporting `canonicalEncMode`, `canonicalDecMode` | 5min | — |
| F30 | Migrate `autodetect_test.go` to `package codec_test`               | 2min | F29    |
| F31 | Migrate `base64_json_test.go` to `package codec_test`              | 2min | F29    |
| F32 | Migrate `cbor_compact_test.go` to `package codec_test`             | 2min | F29    |
| F33 | Migrate `codec_test.go` to `package codec_test`                    | 5min | F29    |
| F34 | Migrate `envelope_test.go` to `package codec_test`                 | 2min | F29    |
| F35 | Migrate `json_contract_test.go` to `package codec_test`            | 3min | F29    |
| F36 | Migrate `resolve_test.go` to `package codec_test`                  | 2min | F29    |
| F37 | Migrate `snaps_clean_test.go` to `package codec_test`              | 2min | F29    |
| F38 | Migrate `streaming_test.go` to `package codec_test` (uses exports) | 5min | F29    |
| F39 | Migrate `transcode_test.go` to `package codec_test`                | 3min | F29    |
| F40 | Re-enable `testpackage` linter in `.golangci.yml`                  | 2min | F30-F39|
| F41 | Verify: `go build ./...` + `go test ./... -race` (v1)              | 3min | F40    |
| F42 | Verify: `GOEXPERIMENT=jsonv2 go test ./... -race` (v2)             | 3min | F41    |

### Tier 6: Parallel Tests

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F43 | Re-enable `paralleltest` in `.golangci.yml` (remove exclusion)      | 2min | F42    |
| F44 | Add `t.Parallel()` to test funcs flagged by paralleltest            | 10min| F43    |
| F45 | Verify: `go test ./... -race` passes with parallel tests            | 3min | F44    |

### Tier 7: Normalizer Tests (depends on Tier 1)

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F46 | Write `TestNormalizeForJSON` — nil, scalar, map, nested, empty      | 8min | F2     |
| F47 | Write depth-cap test — `normalizeForJSON` returns error past max    | 4min | F2     |
| F48 | Write `FuzzNormalizeForJSON` fuzz target                            | 10min| F2     |

### Tier 8: Verification

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F49 | Run `FuzzCBORCodec_Roundtrip` 60s (v1)                              | 2min | —      |
| F50 | Run `FuzzCBORCodec_Roundtrip` 60s (v2)                              | 2min | —      |
| F51 | Run `FuzzTranscodeToJSON` 60s (v1 — exercises normalizer)           | 2min | —      |
| F52 | Run `FuzzTranscodeToJSON` 60s (v2)                                  | 2min | —      |
| F53 | Run `FuzzNormalizeForJSON` 60s (v1)                                 | 2min | F48    |
| F54 | Generate coverage: `go test ./... -coverprofile` (v1)               | 3min | —      |
| F55 | Generate coverage: `GOEXPERIMENT=jsonv2` mode                       | 3min | —      |
| F56 | Add coverage % to FEATURES.md + README                              | 5min | F54,F55|
| F57 | Run `nix build` to verify flake                                     | 5min | —      |
| F58 | Run `nix run .#test` to verify dual-mode flake                      | 5min | F57    |
| F59 | Run `nix run .#lint` to verify lint flake                           | 5min | F57    |
| F60 | Run `nix flake check`                                               | 3min | F57    |

### Tier 9: API Polish

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F61 | Define `SizeResult{JSON, CBOR int}` struct in `size.go`             | 5min | —      |
| F62 | Update `Size` to return `SizeResult`                                | 5min | F61    |
| F63 | Update `autodetect_test.go` callers (2 call sites)                  | 5min | F62    |
| F64 | Update `size.go` doc comment example                                | 3min | F62    |
| F65 | Add `SizeResult` example to `doc.go`                                | 3min | F62    |
| F66 | Add explicit one-way contract note to `TranscodeToJSON` in `doc.go` | 5min | —      |
| F67 | Add `sync.Pool[*bytes.Buffer]` pool helper in `streaming.go` or new file | 10min | —  |

### Tier 10: Benchmarks

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F68 | Add `BenchmarkNormalizeForJSON`                                      | 5min | —      |
| F69 | Add `BenchmarkJSONV1vsV2` (marshal/unmarshal)                        | 8min | —      |
| F70 | Run benchmarks, capture numbers                                     | 5min | F68,F69|
| F71 | Add benchmark numbers to README "19-43% smaller / 25-72% faster"    | 5min | F70    |

### Tier 11: Repo Hygiene

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F72 | Add `CODEOWNERS`                                                    | 2min | —      |
| F73 | Add `SECURITY.md`                                                   | 5min | —      |
| F74 | Add `.github/ISSUE_TEMPLATE/bug_report.yml`                         | 5min | —      |
| F75 | Add `.github/PULL_REQUEST_TEMPLATE.md`                              | 3min | —      |
| F76 | Decide TIMEZONE_HANDLING: accept inline README or create doc        | 5min | —      |
| F77 | Flesh out CONTRIBUTING.md snapshot-update flow                      | 8min | —      |

### Tier 12: Docs Update (after all work)

| #   | Task                                                                | Est  | Deps   |
| --- | ------------------------------------------------------------------- | ---- | ------ |
| F78 | Update CHANGELOG with all changes under `[Unreleased]` or `[0.2.0]` | 10min| F1-F77 |
| F79 | Update FEATURES.md with new features + coverage %                   | 8min | F78    |
| F80 | Update TODO_LIST — remove completed items                           | 5min | F78    |
| F81 | Update AGENTS.md if conventions changed (testpackage, paralleltest) | 5min | F40,F43|
